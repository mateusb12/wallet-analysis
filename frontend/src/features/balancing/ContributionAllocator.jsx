...
import { ArrowRight, Coins, Calculator, Target } from 'lucide-react';
...
const ContributionAllocator = ({ tree, targets }) => {
  ...
  const calculatePlan = () => {
    ...
    const plan = macroDeficits.map((macroItem) => {
      ...
      const subTypes = macroItem.data.subTypes || {};
      let microDeficits = [];
      let totalMicroDeficit = 0;

      Object.entries(subTypes).forEach(([subKey, subData]) => {
        const microTargetPct =
          targets.micro && targets.micro[macroItem.key]
            ? parseFloat(targets.micro[macroItem.key][subKey] || 0)
            : 0;
            ...
            <div key={plan.macro} className="flex flex-col gap-3">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {plan.breakdown.map((item, idx) => (
                  <div
                    key={idx}
                    className="flex flex-col h-full bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 hover:border-emerald-400 dark:hover:border-emerald-500/50 transition-all shadow-sm hover:shadow-md group relative overflow-hidden"
                  >
                    {}
                    ...
                    <div className="p-4 flex flex-col h-full">
                      ...
                      <div className="flex justify-between items-start mb-3">
                        ...
                        <div className="text-right">
                          ...
                          <span className="text-[10px] text-emerald-600/60 font-semibold uppercase tracking-wider">
                            Sugerido
                          </span>
                        </div>
                      </div>
